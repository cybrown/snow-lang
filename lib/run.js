...
function runInstruction(instruction, vm, opcodes) {
    ...
    switch (instruction.kind) {
        ...
        case 'LOAD': {
            ...
            if (opcodes[vm.currentFunction].locals[name]) {
                vm.stack.push(vm.currentLocals[name]);
        ...
        case 'STORE': {
            const name = instruction.args[0];
            if (opcodes[vm.currentFunction].locals[name]) {
                vm.currentLocals[name] = vm.stack[vm.stack.length - 1];
            } else if (opcodes[vm.currentFunction].parameters.indexOf(name) >= 0) {
                const index = opcodes[vm.currentFunction].parameters.indexOf(name);
        ...
        case 'CALL': {
            ...
            vm.callStack.push({
                ...
                currentLocals: vm.currentLocals,
                index: vm.index + 1,
                stack: vm.stack
            });
            vm.index = 0;
...
function runInstructions(instructions, vm, opcodes) {
    ...
    console.log('Warning: reaching end of block without control flow opcode');
}
