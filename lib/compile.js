...
const allCompilers = {
    Program(node, cb, ctx) {
        const functions = node.expressions.filter(node => node.kind === 'Function');
        const expressions = node.expressions.filter(node => node.kind !== 'Function');
        functions.forEach(func => cb(func));
